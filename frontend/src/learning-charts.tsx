  ...
  return <section className="min-w-0" aria-label="知识掌握度">
    ...
    {!entries.length ? <p className="text-sm text-white/55">暂无知识掌握度数据</p> : <>
      {entries.length >= 3 && <svg role="img" aria-label="知识掌握度雷达图，编号对应下方知识点" viewBox="0 0 240 240" className="mx-auto w-full max-w-[280px]">
        ...
        {entries.map(([topic], i) => { const [x, y] = point(i, 1); const [tx, ty] = point(i, 1.19); return <g key={topic}><line x1={center} y1={center} x2={x} y2={y} stroke="rgba(255,255,255,0.16)" /><text x={tx} y={ty} textAnchor="middle" dominantBaseline="middle" fill="#C7B3F5" fontSize="10">{i + 1}</text></g>; })}
        <polygon points={entries.map(([, value], i) => point(i, clamp(value)).join(",")).join(" ")} fill="rgba(115,66,226,0.3)" stroke="#B99DFF" strokeWidth="2" />
        {entries.map(([topic, value], i) => { const [x, y] = point(i, clamp(value)); return <circle key={topic} cx={x} cy={y} r="3" fill="#B99DFF"><title>{topic}：{Math.round(clamp(value) * 100)}%</title></circle>; })}
      </svg>}
    ...
    {!points.length ? <p className="text-sm text-white/55">暂无资源难度信息</p> : <>
      <svg role="img" aria-label="资源序号与难度，紫色虚线为学习者推荐难度" viewBox="0 0 350 170" className="w-full">
        ...
        {points.map((item, i) => <g key={i}>
          ...
          {i > 0 && valid(item.learner_difficulty) && valid(points[i - 1].learner_difficulty) && <line x1={x(i - 1)} y1={y(points[i - 1].learner_difficulty)} x2={x(i)} y2={y(item.learner_difficulty)} stroke="#B99DFF" strokeWidth="2" strokeDasharray="4 4" />}
          {valid(item.learner_difficulty) && <circle cx={x(i)} cy={y(item.learner_difficulty)} r="6" fill="none" stroke="#B99DFF" />}
      ...
      <p className="mb-3 text-xs leading-6 text-white/60">紫色虚线：推荐难度 · 绿：匹配 · 黄：差 1 档 · 红：差 2 档</p>
